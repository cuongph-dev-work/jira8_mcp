...
### Auth Pattern
- **Auth is HTTP-only in tools.** Tools call `loadAndValidateSession()` — cookie session first, then optional HTTP Basic Auth. They never touch Playwright or browser state. SSO login is the `jira-auth-login` CLI only.
...
Requires: GITLAB_TOKEN + .jira/gitlab-projects.json (each mapping requires a non-empty repository `name`)
```
